...
#df_rate <- indiadata %>% 
#  dplyr::group_by(State, type) %>%
#  dplyr::mutate(TotalPrev = lag(cases, default = 0, order_by = date ), DatePrev = lag(date, order_by = date )) %>%
#  dplyr::filter(!is.na(DatePrev)) %>% 
#  dplyr::mutate(DelDay = as.integer(date-DatePrev)) %>%
#  dplyr::mutate( Rate1 = (cases - TotalPrev)*100/(DelDay*cases) ) %>%
#  dplyr::ungroup() 
